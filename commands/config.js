        ...
        if (argc == 0) {
            ...
            if (argv[0] === 'guild' && argv[1] === 'reload') {
                dataAccess.guild.reload(message.guildId).then(() => {
                    ...
                    message.channel.send(`A fatal error occurred whilst reloading server data for \`${message.guildId}\`.`);
                });
                return;
            }
            let embuilder;
            try {
                ...
                embuilder = new EmbedBuilder().setTitle('A fatal error occurred. Please try again.')
            }
            message.channel.send({ embeds: [ embuilder ] });
        }
    }
